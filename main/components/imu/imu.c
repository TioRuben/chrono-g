...
// Software filter tuning (double EMA + optional moving average)
#define FILTER_ENABLE_ACCEL true
#define FILTER_ENABLE_GYRO true
#define FILTER_ALPHA_ACCEL 0.20f
#define FILTER_ALPHA_GYRO 0.10f

// Moving-average pre-filter (N=4)
#define MA_FILTER_SIZE 4

typedef struct
{
    float buf_ax[MA_FILTER_SIZE];
    float buf_ay[MA_FILTER_SIZE];
    float buf_az[MA_FILTER_SIZE];
    float buf_gx[MA_FILTER_SIZE];
    float buf_gy[MA_FILTER_SIZE];
    float buf_gz[MA_FILTER_SIZE];
    uint8_t head;
    bool full;
} ma_filter_t;
...
// Filter state
static ma_filter_t ma_filter = {0};
static imu_data_t filtered_data = {0};
static imu_data_t filtered_data_2 = {0};
...
static void ma_filter_update(ma_filter_t *f, imu_data_t *in, imu_data_t *out)
{
    uint8_t i;
    uint8_t count;
    ...
    f->buf_ax[f->head] = in->accel_x;
    f->buf_ay[f->head] = in->accel_y;
    f->buf_az[f->head] = in->accel_z;
    f->buf_gx[f->head] = in->gyro_x;
    f->buf_gy[f->head] = in->gyro_y;
    f->buf_gz[f->head] = in->gyro_z;

    f->head = (f->head + 1) % MA_FILTER_SIZE;
    if (!f->full && f->head == 0)
    {
        f->full = true;
    }

    count = f->full ? MA_FILTER_SIZE : f->head;
    if (count == 0)
    {
        count = 1;
    }
    ...
    out->accel_x = sum_ax / count;
    out->accel_y = sum_ay / count;
    out->accel_z = sum_az / count;
    out->gyro_x = sum_gx / count;
    out->gyro_y = sum_gy / count;
    out->gyro_z = sum_gz / count;
...
static void apply_software_filter(imu_data_t *raw_data, imu_data_t *filtered_output)
{
    ...
    {
        filtered_data.accel_x = FILTER_ALPHA_ACCEL * raw_data->accel_x + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data.accel_x;
        filtered_data.accel_y = FILTER_ALPHA_ACCEL * raw_data->accel_y + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data.accel_y;
        filtered_data.accel_z = FILTER_ALPHA_ACCEL * raw_data->accel_z + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data.accel_z;
    }
    else
    {
        filtered_data.accel_x = raw_data->accel_x;
        filtered_data.accel_y = raw_data->accel_y;
        filtered_data.accel_z = raw_data->accel_z;
    }

    if (FILTER_ENABLE_GYRO)
    {
        filtered_data.gyro_x = FILTER_ALPHA_GYRO * raw_data->gyro_x + (1.0f - FILTER_ALPHA_GYRO) * filtered_data.gyro_x;
        filtered_data.gyro_y = FILTER_ALPHA_GYRO * raw_data->gyro_y + (1.0f - FILTER_ALPHA_GYRO) * filtered_data.gyro_y;
        filtered_data.gyro_z = FILTER_ALPHA_GYRO * raw_data->gyro_z + (1.0f - FILTER_ALPHA_GYRO) * filtered_data.gyro_z;
    }
    else
    {
        filtered_data.gyro_x = raw_data->gyro_x;
        filtered_data.gyro_y = raw_data->gyro_y;
        filtered_data.gyro_z = raw_data->gyro_z;
    }

    // Stage 2
    if (FILTER_ENABLE_ACCEL)
    {
        filtered_data_2.accel_x = FILTER_ALPHA_ACCEL * filtered_data.accel_x + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data_2.accel_x;
        filtered_data_2.accel_y = FILTER_ALPHA_ACCEL * filtered_data.accel_y + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data_2.accel_y;
        filtered_data_2.accel_z = FILTER_ALPHA_ACCEL * filtered_data.accel_z + (1.0f - FILTER_ALPHA_ACCEL) * filtered_data_2.accel_z;
    }
    else
    {
        filtered_data_2.accel_x = filtered_data.accel_x;
        filtered_data_2.accel_y = filtered_data.accel_y;
        filtered_data_2.accel_z = filtered_data.accel_z;
    }

    if (FILTER_ENABLE_GYRO)
    {
        filtered_data_2.gyro_x = FILTER_ALPHA_GYRO * filtered_data.gyro_x + (1.0f - FILTER_ALPHA_GYRO) * filtered_data_2.gyro_x;
        filtered_data_2.gyro_y = FILTER_ALPHA_GYRO * filtered_data.gyro_y + (1.0f - FILTER_ALPHA_GYRO) * filtered_data_2.gyro_y;
        filtered_data_2.gyro_z = FILTER_ALPHA_GYRO * filtered_data.gyro_z + (1.0f - FILTER_ALPHA_GYRO) * filtered_data_2.gyro_z;
    }
    else
    {
        filtered_data_2.gyro_x = filtered_data.gyro_x;
        filtered_data_2.gyro_y = filtered_data.gyro_y;
        filtered_data_2.gyro_z = filtered_data.gyro_z;
    }

    filtered_data_2.timestamp = raw_data->timestamp;
    *filtered_output = filtered_data_2;
}
...
esp_err_t imu_read_processed(imu_data_t *data)
{
    ...
    // Apply moving-average pre-filter (N=4) and cascaded double EMA
    imu_data_t ma_output;
    ma_filter_update(&ma_filter, data, &ma_output);
    apply_software_filter(&ma_output, data);
...
static void imu_task(void *pvParameters)
{
    ...
    const TickType_t period = pdMS_TO_TICKS(32); // ~31.25 Hz to match accelerometer ODR setting
...
esp_err_t imu_init(i2c_master_bus_handle_t bus_handle, QueueHandle_t imu_queue)
{
    ...
    imu_config_t config = {
        ...
        .gyro_range = QMI8658_GYR_RANGE_512_DPS, // ±512 dps for high-speed rotations
        .accel_odr = QMI8658_ACC_ODR_31_25_HZ,   // Low ODR for prefiltered IMU behavior
        .gyro_odr = QMI8658_GYR_ODR_29_375_HZ,   // Low ODR consistent with accel
        .enable_accel_lpf = true,
        .enable_gyro_lpf = true,
        .lpf_mode = QMI8658_LPF_MODE_1, // 3.93% of ODR (~1.23 Hz at 31.25 Hz) - less aggressive
        .sample_rate_hz = 31,           // ~31 Hz task period for one sample per iteration
