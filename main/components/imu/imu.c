...
// Calibration batch structure for PSRAM
typedef struct
{
...
static void add_calibration_sample(float gyro_x, float gyro_y, float gyro_z)
{
    if (calibration_status == IMU_CALIBRATION_NOT_STARTED)
    {
        ...
        // Allocate first batch in PSRAM
        current_batch = heap_caps_malloc(sizeof(calibration_batch_t), MALLOC_CAP_SPIRAM);
        if (current_batch == NULL)
        {
            ESP_LOGE(TAG, "Failed to allocate calibration batch in PSRAM");
            calibration_status = IMU_CALIBRATION_NOT_STARTED;
            return;
        }
        current_batch->sample_count = 0;

        ESP_LOGI(TAG, "Starting gyro bias calibration. Keep device stationary...");
    }
    ...
    if (calibration_status == IMU_CALIBRATION_IN_PROGRESS && current_batch != NULL)
    {
        ...
        if (batch_index >= CALIBRATION_BATCH_SIZE)
        {
            ...
            {
                // Allocate new batch for next samples
                current_batch = heap_caps_malloc(sizeof(calibration_batch_t), MALLOC_CAP_SPIRAM);
                if (current_batch == NULL)
                {
                    ESP_LOGE(TAG, "Failed to allocate new calibration batch in PSRAM");
...
esp_err_t imu_init(QueueHandle_t imu_queue)
{
    ...
    ret = qmi8658_read_register(&dev, QMI8658_REVISION, &revision, 1);
    if (ret == ESP_OK)
    {
        ESP_LOGI(TAG, "QMI8658 REVISION: 0x%02X", revision);
